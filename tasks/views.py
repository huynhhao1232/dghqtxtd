        ...
        try:
            if mode == TaskAssignForm.ASSIGN_BATCH_DEPARTMENT:
                ...
                if exec_mode == TaskAssignForm.EXEC_ALL_MEMBERS:
                    ...
                    dept_label = ', '.join(d.name for d in departments) or 'Tổ/Nhóm'
                    messages.success(
                        request,
                        f'Đã giao "{base_title}" cho {len(members)} thành viên '
                        f'({dept_label}) — mỗi người một nhiệm vụ riêng.',
                    )
                    return _after_create_redirect(first_task)

                # Mặc định: một Task chung, mỗi tổ một bản phân công độc lập
                with transaction.atomic():
                    task = Task.objects.create(
